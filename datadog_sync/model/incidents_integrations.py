...
class IncidentsIntegrations(BaseResource):
    resource_type = "incidents_integrations"
    resource_config = ResourceConfig(
        ...
        excluded_attributes=[
            ...
            "attributes.status",  # after create, it's always `4`: `indicates manually updated`
            "attributes.metadata.channels.org_id",
            "attributes.metadata.channels.incident_uuid",
        ]
    )
    # Additional IncidentsIntegrations specific attributes
    ...
    integrations_path: str = "/api/v2/incidents/{incident_id}/relationships/integrations"

    def get_resources(self, client: CustomClient) -> List[Dict]:
        # first, get all incidents, then for each incidents, get all incidents integrations
        ...
        for incident in resp_incidents:
            resp += client.paginated_request(client.get)(
                ...
                self.integrations_path.format(incident_id=incident["attributes"]["public_id"]),
                pagination_config=self.pagination_config
            )
        return resp
    ...
    def import_resource(self, _id: Optional[str] = None, resource: Optional[Dict] = None) -> None:
        if _id:
            raise Exception("importing incidents_integrations by id is not supported: we need not only the incidents_integrations id (which we have) but also the parent incident id, which we do not have.")
    ...
    def create_resource(self, _id: str, resource: Dict) -> None:
        ...
        resp = destination_client.post(
            ...
            self.integrations_path.format(incident_id=destination_incident_id),
            payload,
        ).json()
    ...
    def connect_id(self, key: str, r_obj: Dict, resource_to_connect: str) -> Optional[List[str]]:
        return super(IncidentsIntegrations, self).connect_id(key, r_obj, resource_to_connect)
