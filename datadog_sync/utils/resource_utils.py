            ...
            for _id in ids:
                if _id in connection_resources_obj[resource_to_connect]:
                    ...
                    raise ResourceConnectionError(
                        f"missing resource connection. Import {resource_to_connect} with ID: {_id} from source"
                    )
        ...
        if isinstance(r_obj[key], list):
            ...
            if origin == "downtimes":
                ...
                if r_obj[key] in connection_resources_obj[resource_to_connect]:
                    ...
                    raise ResourceConnectionError(
                        f"missing resource connection. Import {resource_to_connect} with ID: {r_obj[key]} from source"
                    )
    else:
        raise ResourceConnectionError(f"missing resource connection. Import {resource_to_connect} from source")
